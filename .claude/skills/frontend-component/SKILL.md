...
## ⚠️ CRITICAL: Clean and Logical Approach

**ALWAYS maintain a clean, logical, and maintainable codebase:**

1. **Use Tailwind utilities properly:**
   - ✅ Prefer Tailwind utilities over custom CSS
   - ✅ Use `@utility` directive for custom utilities (Tailwind v4)
   - ❌ Avoid writing vanilla CSS unless absolutely necessary
   - ❌ Never mix approaches (pick Tailwind OR custom CSS, not both)

2. **Follow established patterns:**
   - ✅ Check existing components before creating new ones
   - ✅ Reuse existing UI primitives from `components/ui/`
   - ✅ Follow the same structure and naming conventions
   - ❌ Don't reinvent the wheel or create duplicate components

3. **Keep code organized:**
   - ✅ One component per file
   - ✅ Clear, descriptive component and prop names
   - ✅ Logical file structure (`components/{domain}/component-name.tsx`)
   - ✅ Group related styles/classes logically in JSX
   - ❌ Don't create messy, hard-to-read className strings

4. **Think before coding:**
   - ✅ Understand the requirement fully before implementing
...
5. **Maintain consistency:**
   - ✅ Use the same spacing scale across components
   - ✅ Follow the same color usage patterns
   - ✅ Keep animation/transition durations consistent
   - ❌ Don't introduce random values or patterns

**Remember:** Clean code is not just about formatting—it's about making logical, consistent decisions that other developers (including future you) can easily understand and maintain.
...
1. **ALWAYS keep code clean and logical** - Think before coding, use Tailwind properly, follow established patterns
2. **Never modify `components/ui/` files** - These are auto-generated by shadcn/ui
3. **Use `cn()` utility** from `@/lib/utils` for className merging
4. **Import from `@/`** - Use absolute imports with @ alias
5. **Type everything** - No implicit `any` types
6. **French UI text** - User-facing text should be in French
7. **API types** - Use types from `@/lib/types` (auto-generated)
8. **Query keys** - Always use the query keys factory
9. **Server Components by default** - Only add `'use client'` when needed
10. **Semantic HTML** - Use proper tags (nav, main, article, section, aside)
11. **Accessibility first** - Not an afterthought
12. **Tailwind over custom CSS** - Use `@utility` for custom utilities, avoid vanilla CSS
