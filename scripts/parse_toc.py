...
Usage:
    python scripts/parse_toc.py <path>   # path vers un dossier (cherche toc.ncx récursivement) ou vers un fichier toc.ncx
"""
...
def find_toc(books_dir: Path) -> Path:
    ...
    if not hits:
        sys.exit(f"ERROR: no toc.ncx found under {books_dir}")
    if len(hits) > 1:
...
def main():
    if len(sys.argv) < 2:
        sys.exit("Usage: python scripts/parse_toc.py <path>  # dossier ou fichier toc.ncx")
